...
%% By Kihong Shin, Prof Joseph Hammond (2008)
%% revised by Chiara Grappasonni (2012) 
%% revised by Valerio Camerini and Elisabetta Conti (2016)
...
% Welch method percentage overlap
perc_ovlp = 50;

% win_string : hanning or rectwin, string variable
win_string = 'rectwin';
...
fs = 100; 
T1 = 10; % Time-Length for the Segments 
t1 = 0:1/fs:(T1-1/fs);
...
% Generate the measurement noise acting on input and output for case (c)
nx = 0.5*randn(size(x)); % nx=0 for case (a)
ny = 0.5*randn(size(y)); % ny=0 for case (b)

% Window function
if strcmp(win_string,'hanning')
    win_fun = hanning(T1*fs);
elseif strcmp(win_string,'rectwin')
    win_fun = rectwin(T1*fs);
else
    disp('ERROR: Set window function variable as hanning or rectwin!')
end

% Calculate the (one-sided) spectral density functions using the segment
% averaging method with hanning window, 50% overlap.

% Then calculate the FRF and the coherence function.  
disp('Wait patiently...');

% Case a) noise output only
ya = y+ny;
[Gxx, f] = cpsd(x(1:T*fs),x(1:T*fs), win_fun, round(T1*perc_ovlp*fs/100), T1*fs, fs);
[Gyy, f] = cpsd(ya(1:T*fs),ya(1:T*fs), win_fun, round(T1*perc_ovlp*fs/100), T1*fs, fs);
[Gxy, f] = cpsd(x(1:T*fs),ya(1:T*fs), win_fun, round(T1*perc_ovlp*fs/100), T1*fs, fs);
[Gyx, f] = cpsd(ya(1:T*fs),x(1:T*fs), win_fun, round(T1*perc_ovlp*fs/100), T1*fs, fs);

% round -> gives the integer number 
...
% Case b) noise input only
xb = x+nx; 
[Gxx, f] = cpsd(xb(1:T*fs),xb(1:T*fs), win_fun, round(T1*perc_ovlp*fs/100), T1*fs, fs);
[Gyy, f] = cpsd(y(1:T*fs),y(1:T*fs), win_fun, round(T1*perc_ovlp*fs/100), T1*fs, fs);
[Gxy, f] = cpsd(xb(1:T*fs),y(1:T*fs), win_fun, round(T1*perc_ovlp*fs/100), T1*fs, fs);
[Gyx, f] = cpsd(y(1:T*fs),xb(1:T*fs), win_fun, round(T1*perc_ovlp*fs/100), T1*fs, fs);
...
% Case c) noise input - output
% xc=xb; yc=ya;
[Gxx, f] = cpsd(xb(1:T*fs),xb(1:T*fs), win_fun,round(T1*perc_ovlp*fs/100), T1*fs, fs);
[Gyy, f] = cpsd(ya(1:T*fs),ya(1:T*fs), win_fun,round(T1*perc_ovlp*fs/100), T1*fs, fs);
[Gxy, f] = cpsd(xb(1:T*fs),ya(1:T*fs), win_fun,round(T1*perc_ovlp*fs/100), T1*fs, fs);
[Gyx, f] = cpsd(ya(1:T*fs),xb(1:T*fs), win_fun,round(T1*perc_ovlp*fs/100), T1*fs, fs);
...
legend('Noise only Output','Noise only Input','Noise Input-Output')
clc
